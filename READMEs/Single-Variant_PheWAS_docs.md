...
| `snplist` | Input Files | Tab-separated file of SNPs to test |
| `group_file_prefix` | Input Files | SAIGE gene annotation file prefix (for singles context) |
...
| `pheno_descriptions_file` | Optional | Path to a CSV file with columns `PHENO`, `DESCRIPTION`, `CATEGORY`. Used to label and group phenotypes in PheWAS plots. |
...
**Example `pheno_descriptions_file` format:**
```
PHENO,DESCRIPTION,CATEGORY
Phe401.1,Essential hypertension,Circulatory system
Phe250.2,Type 2 diabetes,Endocrine/metabolic
```
...
| `snplist` | **Required** | Tab-separated file with one row per variant of interest: `chromosome`, `start`, `stop`, `rsid`. Used to extract the target variants for Step 2. |
...
| `group_file_prefix` | Optional | Prefix for chromosome-separated SAIGE gene annotation files. Required if running gene-based tests alongside single-variant tests. |

**Example `snplist` format:**
```
1	100000	100001	rs123456
7	117548628	117548629	rs10272438
```
...
| `sex_strat_cohort_list` | *(required)* | Subset of `cohort_list` that are sex-stratified (female-only or male-only cohorts). |
...
| `chromosome_list` | *(required)* | List of chromosomes to include in Step 2. |
| `cat_covars` | `[]` | Categorical covariate column names (e.g. `[“SEX”]`). Used for non-sex-stratified cohorts. |
| `cont_covars` | `[]` | Continuous covariate column names (e.g. `[“AGE”,”PC1”]`). Used for non-sex-stratified cohorts. |
...
| `geno` | `0.01` | Maximum per-variant missingness for Step 1 variant QC. |
...
| `LOCO` | `”FALSE”` | Leave-one-chromosome-out for the null model. Typically `”FALSE”` for targeted variant tests. |
...
Step 2 tests each target variant for association across all phenotypes using the null model from Step 1.

### Beginner (Most Commonly Changed)

| Parameter | Default | Description |
|-----------|---------|-------------|
| `min_mac` | `0.5` | Minimum minor allele count for a variant to be tested. |
| `min_maf` | `0` | Minimum minor allele frequency for a variant to be tested. |
...
| `grouptest_annotation` | *(see config)* | Comma-separated annotation groups for gene-based tests (if using group tests alongside single-variant tests). |
| `grouptest_maf` | `”0.01”` | Comma-separated MAF cutoffs for gene burden tests. |
...
| Parameter | Default | Description |
|-----------|---------|-------------|
| `p_cutoff_summarize` | `0.001` | P-value threshold for the top-hits summary table. |
...
params {
    ...
    // default paths are for BIOBANK Geno data (PLINK)
    step1_plink_prefix  = "/path/to/data/BIOBANK-Release-VERSION_genetic_genotype"

    // default paths are for BIOBANK Geno data (BGEN)
    step2_bgen_prefix  = "/path/to/data/BIOBANK-Release-VERSION_genetic_imputed-topmed-r2_10Ksubset_"
    samplefile = "/path/to/data/BIOBANK-Release-VERSION_genetic_imputed-topmed-r2_bgen.sample"
    step2_pgen_prefix = "/path/to/data/BIOBANK-Release-VERSION_genetic_imputed-topmed-r2_" 
