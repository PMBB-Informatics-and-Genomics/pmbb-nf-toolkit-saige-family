...
if(step2_fam):
    if step2_fam.endswith('.psam'):
        _psam = pd.read_table(step2_fam, header=0, sep='\\s+', dtype=str)
        _psam.columns = [c.lstrip('#') for c in _psam.columns]
        step2_samples = pd.Index(_psam['IID'])
    else:
