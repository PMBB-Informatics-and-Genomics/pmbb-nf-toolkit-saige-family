...
"""
    ...
    */
    ...
    all_plot_pngs = singles_plots.map { pngs, csvs -> pngs }.flatten()
        .concat(regions_plots.map { pngs, csvs -> pngs }.flatten())
        .concat(preprocessing_output[6].flatten())
        .collect()

    exwas_manifest_script = "${moduleDir}/../scripts/generate_saige_exwas_manifest.py"
    exwas_report_script   = "${moduleDir}/../scripts/generate_saige_exwas_reports.py"
    make_saige_exwas_report(
        all_plot_pngs,
        singles_summary,
        regions_summary,
